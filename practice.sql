...
SELECT employee_name as "Employee Name", department_name as "Department Name" FROM employees JOIN departments USING(department_id);

SELECT department_name as "Department Name", round(avg(salary)) as "Salary" FROM departments JOIN employees USING(department_id) GROUP BY department_name 

SELECT department_name as "Department Name", round(avg(salary)) as avg_salary FROM departments JOIN employees USING(department_id) GROUP BY department_name ORDER BY avg_salary DESC LIMIT 1;
...
SELECT extract(MONTH FROM order_date) as "Order Month", SUM(total_amount) FROM orders WHERE extract(YEAR FROM order_date)=2022 GROUP BY "Order Month"